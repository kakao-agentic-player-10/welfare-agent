...
| `KAKAO_REST_API_KEY` | 방문 기관 위치 검색. `KAKAO_LOCAL_PROXY_URL`을 쓰지 않을 때 필요 |
| `KAKAO_LOCAL_PROXY_URL` | Kakao 키를 런타임에 넣을 수 없는 배포 환경에서 사용할 Kakao Local 프록시 URL |
| `KAKAO_LOCAL_PROXY_TOKEN` | Kakao Local 프록시 인증 토큰. 런타임 환경변수로만 설정하고 Dockerfile/레포에는 넣지 않음 |
...
`OPENAI_API_KEY`, `KAKAO_REST_API_KEY`, `OPENAI_EMBEDDING_PROXY_TOKEN`, `KAKAO_LOCAL_PROXY_TOKEN`은 Dockerfile, GitHub, 공개 문서에 넣지 않습니다.
PlayMCP in KC처럼 런타임 환경변수 입력이 어려운 경우 별도 임베딩 프록시에 OpenAI 키를 보관하고,
이 서버에는 공개 가능한 프록시 URL만 설정합니다.
...
현재 PlayMCP in KC 배포용 기본 Kakao Local 프록시 URL:

```text
https://playmcp-embedding-proxy.onrender.com/v1/kakao/local/search
```
